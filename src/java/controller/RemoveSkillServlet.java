    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if ("offered".equals(type)) {
                success = userSkillsDao.removeOfferedSkill(user.getUserId(), skillId);
            } else if ("wanted".equals(type)) {
                success = userSkillsDao.removeWantedSkill(user.getUserId(), skillId);
            }
            
            if (success) {
                response.sendRedirect("profile.jsp");
            } else {
                response.sendRedirect("profile.jsp?error=Failed to remove skill");
            }
